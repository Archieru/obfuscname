class GetNameController < ApplicationController
  def index
    seed = Digest::MD5.hexdigest(params[:seed])
    name_seed = "0x#{seed[0,15]}".to_i(16)
    surname_seed = "0x#{seed[16,31]}".to_i(16)
    ...
    render json: {status: 'SUCCESS', message: "hello, #{return_name} #{return_surname}"}, status: :ok
  end
end
